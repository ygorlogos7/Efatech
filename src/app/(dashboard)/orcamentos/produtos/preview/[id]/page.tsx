...
export default async function PreviewOrcamentoProdutoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!success || !orcamento) {
    return (
      <div className="p-10 text-center">
        ...
        <Link href="/orcamentos/produtos" className="mt-4 inline-block text-blue-600 hover:underline">Voltar para a lista</Link>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-md shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2">
          <Link href="/orcamentos/produtos" className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-600 text-sm font-medium rounded-md border border-gray-200 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Voltar
          </Link>
          <Link href={`/orcamentos/produtos/edit/${orcamento.Id}`} className="flex items-center gap-2 px-4 py-2 bg-[#f39c12] hover:bg-[#db8b0b] text-white text-sm font-medium rounded-md shadow-sm transition-colors">
