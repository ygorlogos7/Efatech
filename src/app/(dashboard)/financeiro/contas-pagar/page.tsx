...
export default function ContasPagarPage() {
  ...
  const handleAdd = (formData: FormData) => {
    startTransition(async () => {
      const r = await createContaPagar(formData);
      if (r.success) {
        setIsAdding(false);
        const u = await getContasPagar();
        if (u.success) setItems(u.data as any[]);
      } else {
        alert(r.error || "Erro ao salvar conta.");
      }
    });
  };
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-end animate-in slide-in-from-top duration-200">
          <div className="md:col-span-2">
            ...
            <input type="text" name="Descricao" required className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-red-500 focus:ring-1 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Valor (R$) *</label>
            <input type="number" name="Valor" required step="0.01" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-red-500 focus:ring-1 focus:ring-red-500" />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Vencimento *</label>
            <input type="date" name="Vencimento" required className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-red-500 focus:ring-1 focus:ring-red-500" />
          </div>
          <div className="flex gap-2">
            <button type="submit" className="flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded text-sm"><Check className="w-3 h-3"/>Salvar</button>
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              <tr key={item.Id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="py-3 px-4 text-right font-bold text-red-600">R$ {item.Valor.toFixed(2).replace(".", ",")}</td>
                ...
                <td className="py-3 px-4 text-center">
                  <span className={`inline-block px-2.5 py-1 text-[11px] font-bold rounded border ${item.Pagamento ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-600 border-red-200"}`}>
