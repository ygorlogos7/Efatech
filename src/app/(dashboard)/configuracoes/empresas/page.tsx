...
import { PlusCircle, SearchIcon, Edit2, Package, Home, Eye } from "lucide-react";
import { getEmpresas } from "@/actions/empresas";
import { DeleteEmpresaButton } from "@/components/forms/DeleteEmpresaButton";

export default async function EmpresasLojasPage({
  searchParams,
}: {
  ...
  const { success, data: empresas } = await getEmpresas(pesquisa, "interno");
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-md shadow-sm border border-gray-200 gap-4">
        <div className="flex flex-col gap-1">
          <h2 className="text-gray-900 font-bold text-xl sm:text-2xl mb-0">Empresas / Lojas</h2>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <Link href="/configuracoes/empresas/create" className="flex items-center justify-center gap-1.5 bg-[#00a65a] hover:bg-green-600 transition-colors text-white text-sm font-bold px-4 py-2 rounded shadow-sm flex-grow sm:flex-grow-0" title="Cadastrar nova empresa">
