...
export default async function EditVendaServicosPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!success || !data) return (
    <div className="p-10 text-center">
      ...
      <Link href="/vendas/servicos" className="mt-4 inline-block text-blue-600 hover:underline">Voltar</Link>
    </div>
  );
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-2xl font-bold text-gray-900">Editar Venda {data.Numero} — Serviços</h3>
      ...
      <VendaForm tipo="servicos" initialData={data} isReadOnly={false} />
    </div>
  );
}
