...
export default async function ClientesPage({
  ...
  // Fetch from DB via server action
  const { success, data: clientes, error } = await getClientes(pesquisa);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-md shadow-sm border border-gray-200 gap-4">
        <div className="flex flex-col gap-1">
          <h2 className="text-gray-900 font-bold text-xl sm:text-2xl mb-0">Clientes</h2>
      ...
      <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-100">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left align-middle border-collapse min-w-[800px]">
            ...
            <tbody>
              {!success || !clientes || clientes.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-16 text-gray-500">
                    <Box className="w-12 h-12 mx-auto mb-3 opacity-30" />
                    <h5 className="text-lg font-medium text-gray-700">Nenhum cliente encontrado</h5>
                ...
                clientes.map((cliente) => (
                  <tr key={cliente.Id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-6 font-bold text-gray-900">{cliente.Nome}</td>
                    <td className="py-3 px-4 text-gray-600">{cliente.CPFCNPJ || "-"}</td>
                    <td className="py-3 px-4 text-gray-600">{cliente.Telefone || "-"}</td>
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`inline-block px-2.5 py-1 text-[11px] font-medium rounded border ${
                          cliente.Ativo
                    ...
                    <td className="py-3 px-6 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Link href={`/cadastros/clientes/preview/${cliente.Id}`} className="p-1.5 text-blue-400 hover:text-blue-600 border border-blue-200 rounded hover:bg-blue-50 transition-colors" title="Visualizar">
                          <SearchIcon className="w-4 h-4" />
                        </Link>
                        <Link href={`/cadastros/clientes/edit/${cliente.Id}`} className="p-1.5 text-blue-500 hover:text-blue-700 border border-blue-200 rounded hover:bg-blue-50 transition-colors" title="Editar">
