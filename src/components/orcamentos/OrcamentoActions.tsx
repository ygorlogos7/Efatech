...
export function OrcamentoActions({ item, baseUrl, tipo = "servicos", situacoes = [] }: OrcamentoActionsProps) {
  ...
  const handleSendEmail = () => {
    const email = item.Clientes?.Email || item.Cliente?.Email;
    if (!email) {
    ...
    startTransition(async () => {
      const subject = `Orçamento #${item.Numero} - Efatech PRO`;
      const pdfLink = `${baseUrl}/orcamentos/${tipo}/print/${item.Id}/a4`;
      const html = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; padding: 20px;">
          <h2 style="color: #38b473;">Efatech PRO</h2>
          <p>Olá <strong>${item.Clientes?.Nome || item.Cliente?.Nome || ""}</strong>,</p>
          <p>Seu Orçamento <strong>#${item.Numero}</strong> está pronto.</p>
          <p>Você pode visualizar e baixar o documento em PDF clicando no botão abaixo:</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${pdfLink}" style="background-color: #38b473; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-weight: bold;">Visualizar Orçamento</a>
          </div>
  ...
  const actions: ActionItem[] = [
    { label: "Link de cobrança", icon: <DollarSign className="w-4 h-4" /> },
    {
      ...
      subItems: [
        { label: "Formato A4", href: `/orcamentos/${tipo}/print/${item.Id}/a4` },
    ...
    },
    {
      ...
      subItems: [
        ...
        { 
          ...
          href: getWhatsAppLink(item.Clientes?.TelefoneCelular || item.Clientes?.Telefone || item.Clientes?.TelefoneComercial, `Olá ${item.Clientes?.Nome || ""}, seu orçamento de ${tipo === 'produtos' ? 'produtos' : 'serviços'} #${item.Numero} está pronto. Você pode visualizá-lo e baixá-lo em PDF através deste link: ${baseUrl}/orcamentos/${tipo}/print/${item.Id}/a4`) || undefined,
