    ...
    <header className="h-[60px] bg-[#1a1c23] flex items-center justify-between px-4 sm:px-6 text-white shrink-0 shadow-md relative z-[100]">
      ...
      <div className="flex items-center gap-5" ref={dropdownRef}>
        ...
        <div className="relative">
          ...
          {activeDropdown === 'grid' && (
            <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-100 dark:border-slate-700 p-4 text-gray-800 dark:text-slate-100 animate-in fade-in zoom-in duration-200">
              <h3 className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-4 px-2">Acesso Rápido</h3>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <Link key={i} href={app.href} className="flex flex-col items-center gap-1.5 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
        ...
        <div className="relative">
          ...
          {activeDropdown === 'bell' && (
            <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden text-gray-800 dark:text-slate-100 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="p-4 border-b border-gray-50 dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-700/30">
              ...
              <div className="max-h-80 overflow-y-auto">
                {[
                  ...
                  <div key={i} className="p-4 hover:bg-gray-50 dark:hover:bg-slate-700 border-b border-gray-50 dark:border-slate-700 last:border-0 cursor-pointer transition-colors">
                    <div className="flex gap-3">
                      <span className="text-xl">{notif.icon}</span>
                      <div>
                        <p className="text-xs font-medium text-gray-700 dark:text-slate-200">{notif.text}</p>
                        <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">{notif.time}</p>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              <button className="w-full py-3 text-center text-xs font-bold text-gray-400 dark:text-slate-500 hover:bg-gray-50 dark:hover:bg-slate-700 hover:text-[#38b473] transition-colors">
        ...
        <div className="flex items-center gap-3 pl-3 border-l border-gray-700 relative">
          ...
          {activeDropdown === 'user' && (
            <div className="absolute right-0 top-12 w-56 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden text-gray-800 dark:text-slate-100 animate-in fade-in zoom-in-95 duration-150">
              <div className="p-4 bg-gray-50/50 dark:bg-slate-700/30 border-b border-gray-50 dark:border-slate-700">
                <p className="text-sm font-bold truncate">{userName || "Usuário"}</p>
                <p className="text-[10px] text-gray-500 dark:text-slate-400 truncate">{userEmail || "email@efatech.com"}</p>
              </div>
              <div className="p-1">
                <Link href="/configuracoes/usuarios" className="flex items-center gap-2.5 px-3 py-2 text-xs font-medium rounded-md hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white transition-colors">
                  <User className="w-4 h-4" /> Meu Perfil
                </Link>
                <Link href="/configuracoes/gerais" className="flex items-center gap-2.5 px-3 py-2 text-xs font-medium rounded-md hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white transition-colors">
                  <Settings className="w-4 h-4" /> Configurações
                </Link>
                <Link href="/configuracoes/plano" className="flex items-center gap-2.5 px-3 py-2 text-xs font-medium rounded-md hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white transition-colors border-b border-gray-50 dark:border-slate-700 pb-3 mb-1">
