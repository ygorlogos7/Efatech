      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-100 overflow-hidden">
            ...
            {!success || !items || items.length === 0 ? (
              <tr>
                <td colSpan={tipo === "balcao" ? 7 : 8} className="text-center py-16 text-gray-500">
                  ...
                  <h5 className="text-lg font-medium text-gray-700">Nenhuma venda encontrada.</h5>
                </td>
              </tr>
            ) : (
                ...
                <tr key={item.Id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-6 text-right">
                    <div className="flex justify-end items-center gap-1">
                      ...
                      <VendaActions item={item} baseUrl={getBaseUrl()} tipo={tipo} />
                    </div>
                  </td>
                </tr>
              ))
        ...
        <div className="px-4 sm:px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between bg-white rounded-b-md gap-4">
          ...
          <div className="flex items-center -space-x-px">
            <Link
              href={`/vendas/${tipo}?page=${Math.max(1, page - 1)}${pesquisa ? `&pesquisa=${pesquisa}` : ""}`}
            ...
            {Array.from({ length: Math.ceil(total / 20) }).map((_, i) => {
              ...
              if (p === 1 || p === Math.ceil(total / 20) || (p >= page - 1 && p <= page + 1)) {
                return (
                  <Link
                    key={p}
                    href={`/vendas/${tipo}?page=${p}${pesquisa ? `&pesquisa=${pesquisa}` : ""}`}
            ...
            <Link
              href={`/vendas/${tipo}?page=${Math.min(Math.ceil(total / 20), page + 1)}${pesquisa ? `&pesquisa=${pesquisa}` : ""}`}
